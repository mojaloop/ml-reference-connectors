...
describe('SDK Scheme Adapter Unit Tests', () => {
    ...
    test('SDK-Scheme Adapter updateTransfer - should fail when given an invalid ID and returns a 404 error', async () => {
          // arrange
          const continueTransfer: TSDKTransferContinuationRequest = {
              acceptQuote: true,
          };
          ...
          // assert
          await expect(sdkClient.updateTransfer(continueTransfer, "invalid-id")).rejects.toThrow('Request failed with status code 404');
      });
