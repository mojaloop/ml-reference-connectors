...
export const aggregateFactory = <D>(
    ...
    return new DFSPCoreConnectorAggregate<D>(
        sdkClient,
        cbsClient,
        cbsConfig,
        logger
...
export class DFSPCoreConnectorAggregate<D> implements IDFSPCoreConnectorAggregate<D> {
    ...
    private checkSendAmounts(transfer: TtransferRequest): TValidationResponse {
        ...
        if (!transfer.quote.payeeReceiveAmount || !transfer.quote.payeeFspFeeAmount) {
            throw AggregateError.notEnoughInformationError("transfer.quote.payeeReceiveAmount or !transfer.quote.payeeFspFeeAmount not defined", "5000");
        }
    ...
    private checkReceiveAmounts(transfer: TtransferRequest): TValidationResponse {
        ...
        if (!transfer.quote.payeeFspFeeAmount || !transfer.quote.payeeReceiveAmount) {
            throw AggregateError.notEnoughInformationError("transfer.quote.payeeFspFeeAmount or transfer.quote.payeeReceiveAmount not defined", "5000");
        }
        if (
            parseFloat(transfer.amount) !==
            parseFloat(transfer.quote.transferAmount) -
    ...
    private async handleSendTransferRes(res: TSDKOutboundTransferResponse, homeTransactionId: string | undefined): Promise<TCbsSendMoneyResponse> {
        /*
            check fxQuote
    ...
    private async handleReceiveTransferRes(res: TSDKOutboundTransferResponse, homeTransactionId: string | undefined): Promise<TCbsSendMoneyResponse> {
        /*
    ...
    private validateConversionTerms(transferRes: TSDKOutboundTransferResponse): TValidationResponse {
        ...
        if (
            ...
            result = false;
            message.push(`cbsConfig.CURRENCY ${this.cbsConfig.CURRENCY} did not match currency returned in transferRes.fxQuotesResponse?.body.conversionTerms.sourceAmount.currency ${transferRes.fxQuoteResponse?.body.conversionTerms.sourceAmount.currency}`);
        }
    ...
    private getTCbsSendMoneyResponse(transfer: TSDKOutboundTransferResponse, homeTransactionId: string | undefined): TCbsSendMoneyResponse {
    ...
    private async getTSDKOutboundTransferRequest(transfer: TCbsSendMoneyRequest | TCbsMerchantPaymentRequest, amountType: "SEND" | "RECEIVE"): Promise<TSDKOutboundTransferRequest> {
        return {
    ...
    private getOutboundTransferExtensionList(sendMoneyRequestPayload: TCbsSendMoneyRequest | TCbsMerchantPaymentRequest): TPayerExtensionListEntry[] | undefined {
